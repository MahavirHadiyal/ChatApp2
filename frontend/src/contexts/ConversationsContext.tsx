...
export const useConversationsContext = () => {
    ...
    if (!context) throw new Error("useConversations context must be use within ConversationsProvider");
    return context;
}
...
export const ConversationsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const handleNewConversation = (conversation: Conversation) => {
        ...
        toast.success(`you and ${conversation.friend.username} are now friend`)
    }

    const handleErrorNewConversation = () => toast.error("Unable to add conversation");

    // ✅ Fixed: was named handleErrorConversationMarkasRead but never correctly wired
