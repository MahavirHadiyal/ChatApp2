...
const registerSchema = z.object({  // registeer frontend achema using zod 
...
type RegisterFormData = z.infer<typeof registerSchema> // note here this line 
...
const RegisterForm: React.FC<RegisterFormProps> = ({ onSwitch }) => {
    ...
    const mutation = useMutation({
        ...
        onError: (error) => {
            const msg = error.response?.data?.msssage || "Registration failed"
            toast.error(msg)
        },
    })
    ...
    return <>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            >
                {mutation.isPending ? <Loader2 className="animate-spins size-5" /> : "Create Account"}
